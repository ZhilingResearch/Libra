...
def synchronization_loop(args):
    try:
        for _ in range(1):
            ...
            # 添加车辆
            traci.route.add("trip", ["24","-30"])  # 先设置路由，第一个是路由id
            traci.vehicle.add("2", "trip")  # 添加车辆
            # traci.vehicle.changeTarget("2","-30")  # 更改车辆终点，会自动计算最优路径

            # 自己加了代码可能会报错，例如获取数据时车子已经没了
            while traci.simulation.getMinExpectedNumber() > 0:
                synchronization.tick()

                all_vehicle_id = traci.vehicle.getIDList()  # 返回所有车的id列表
                result=traci.vehicle.getSubscriptionResults(all_vehicle_id[0])  # 详细看sumo_simulation.py文件400行
                speed = result[traci.constants.VAR_SPEED]
                print("车子速度", speed)

                # if traci.vehicle.getLaneID("0")=="24_4":  # 或者车子所在edge的lane_id
                #     traci.vehicle.changeLane("0",0,4)  # 变道，“0”是车子id，0是该edge的一车道（目标车道），4是持续时间
                #     traci.vehicle.changeSublane("0", -1)  # 变道，“0”是车子id，-1表示向右变道，1表示向左，不确定-2是否表示变两根道

                # print(traci.vehicle.getLaneChangeMode("0"))  # 获取车子换道模型，输出的是换道代码
                # traci.vehicle.setLaneChangeMode("0",1621)
                # 获取车子速度，以及其他的，可看result
