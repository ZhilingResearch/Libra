...
class BehaviorAgent(Agent):
    ...
    # 更新车子车辆信息
    def update_information(self, vehicle, speed_limit):
        """
    ...
    def set_destination(self, start_location, end_location, clean=False):
        ...
        route_trace = self._trace_route(self.start_waypoint, self.end_waypoint)[9:]
        self._local_planner.set_global_plan(route_trace, clean)
        return route_trace

    def set_destination2(self, route_trace, clean=False):
        """
                This method creates a list of waypoints from agent's position to destination location
                based on the route returned by the global router.
